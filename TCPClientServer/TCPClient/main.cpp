...
public:
	...
	{
		std::lock_guard<std::recursive_mutex> lockClass(m_mutex);
...
private:
	...
	void readHeader()
	{
		...
		boost::asio::async_read(m_socket, boost::asio::buffer(m_readMessage.getBuffer(), HEADER_LENGTH),
		                        [this](boost::system::error_code ec, std::size_t length) {
			                        ...
			                        std::cout << "size1 : " << m_readMessage.getLength() << std::endl;

			                        if (!ec && m_readMessage.getLengthFromHeader()) {
				                        std::cout << "size2 : " << m_readMessage.getLength() << std::endl;
				                        readBody();
			                        } else {
				                        m_socket.close();
			                        }
	...
	void readBody()
	{
		boost::asio::async_read(m_socket, boost::asio::buffer(m_readMessage.getBodyBuffer(), m_readMessage.getLength()),
		                        [this](boost::system::error_code ec, std::size_t length) {
			                        std::cout << "size3 : " << (m_readMessage.getLength()) << std::endl;
			                        if (!ec) {
